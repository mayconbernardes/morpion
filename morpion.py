...
def afficher_grille(grille, message=""):
    ...
    plt.xlim(0, 3)
    plt.ylim(-0.5, 3.5)  # Ajuster l'espace pour afficher le message en-dessous de la grille
    plt.gca().set_aspect('equal', adjustable='box')
    plt.axis('off')

    # Afficher le message s'il existe
    if message:
        plt.text(1.5, -0.2, message, fontsize=15, ha='center', color='red')
    ...
    while True:
        ...
        if case_valide(grille, ligne, colonne):
            ...
            if verifier_victoire(grille, joueur):
                afficher_grille(grille, f"Félicitations ! Le joueur {joueur} a gagné !")
                break

            # Vérifier le match nul
            if verifier_match_nul(grille):
                afficher_grille(grille, "Match nul !")
                break
